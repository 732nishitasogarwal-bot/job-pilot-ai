...
function buildProfileBlock(p: {
  ...
  return [
    ...
    p.projects ? `Projects:\n${p.projects}` : "",
    p.masterResumeText
      ? `Student's own resume (same facts, untailored):\n${p.masterResumeText.slice(0, 3000)}`
      : "",
...
/** Human-triggered tailoring. Gated at Shortlisted+. */
export const tailor = action({
  args: { jobId: v.id("jobs") },
  handler: async (ctx, { jobId }): Promise<TailorReport> => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Sign in required.");
    return await runTailor(ctx, userId, jobId);
  },
});

/**
 * The same work for autopilot and the daily scheduler, which have no auth
 * identity — hence the explicit userId.
 */
export const tailorJobForUser = internalAction({
  args: { userId: v.id("users"), jobId: v.id("jobs") },
  handler: async (ctx, { userId, jobId }): Promise<TailorReport> =>
    await runTailor(ctx, userId, jobId),
});
...
async function runTailor(
  ...
  const { profile, job } = await ctx.runQuery(
    internal.private.getProfileAndJobForUser,
    { userId, jobId },
  );
  ...
  await ctx.runMutation(internal.private.saveTailoredForUser, {
    userId,
    jobId,
    resumeHtml,
